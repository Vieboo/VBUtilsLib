...
//        }
    ...
    /**
     * 获取AndroidManifest.xml里面的meta值 (字符串类型)
     * @param key
     * @return
     */
    public static String getManifestMetaDataStr(Context context, String key) {
        String result = "";
        try {
    ...
    /**
     * 获取AndroidManifest.xml里面的meta值 (int类型)
     * @param key
     * @return
     */
    public static int getManifestMetaDataInt(Context context, String key, int defaultValue) {
        int result = -1;
        try {
            result = context.getPackageManager()
                    .getApplicationInfo(context.getPackageName(),
    ...
    /**
     * 获取已安装Apk文件的源Apk文件 如：/data/app/**.apk
     *
     * @param context
     * @param packageName
     * @return
